...
class ArangoDB_Networkx_Adapter(ADBNX_Adapter):
    ...
    def create_networkx_graph(
        ...
        for col, atribs in graph_attributes["vertexCollections"].items():
            for v in self.__fetch_arangodb_docs(col, atribs, is_keep, query_options):
                self.__insert_networkx_node(v["_id"], v, col, nodes)
        ...
        for col, atribs in graph_attributes["edgeCollections"].items():
            for e in self.__fetch_arangodb_docs(col, atribs, is_keep, query_options):
                self.__insert_networkx_edge(e, col, edges)
    ...
    def create_arangodb_graph(
        ...
        for node_id, node in nx_graph.nodes(data=True):
            ...
            node["_id"] = col + "/" + key
            self.__insert_arangodb_vertex(node_id, node, col, key, adb_documents)
        ...
        for from_node_id, to_node_id, edge in nx_graph.edges(data=True):
            ...
            self.__insert_arangodb_edge(edge, from_node, to_node, col, adb_documents)
    ...
    def __insert_networkx_node(self, adb_id: str, node: dict, col: str, nodes: list):
    ...
    def __insert_networkx_edge(self, edge: dict, col: str, edges: list):
    ...
    def __insert_arangodb_vertex(self, id, v: dict, col: str, key: str, vertices: dict):
